...
function pickVoice() {
  ...
  const preferred = ["Google UK English Male", "Google US English", "Daniel", "Alex", "Arthur", "Samantha", "Google UK English Female"];
  for (const name of preferred) {
    const v = voices.find(v => v.name === name);
    if (v) return v;
  }
  return voices.find(v => v.lang === "en-GB") || voices.find(v => v.lang === "en-US") || voices.find(v => v.lang.startsWith("en")) || voices[0];
}
    ...
    function draw() {
      ...
      if (state === "speaking") {
        ...
        for (let i = 0; i < 3; i++) {
          ...
          ctx.beginPath(); ctx.arc(cx + 12 * Math.cos(a), (cy - faceR - 14) + 5 * Math.sin(a), 2.5 + Math.sin(a), 0, Math.PI * 2);
          ctx.fillStyle = col.glow + "cc"; ctx.fill();
        }
      } else {
        ctx.beginPath(); ctx.moveTo(mouthCX - mouthW, mouthCY);
...
export default function ATLASPage() {
  ...
  // ── speakText — fixed for mobile (user gesture already happened via mic tap) ─
