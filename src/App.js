...
// Approximate SVG polygon coordinates for French regions (500×560 viewBox)
const FR_REGIONS_SVG = {
  "Hauts-de-France":             { points:"215,22 320,22 345,32 335,105 295,112 248,112 218,95 218,42", cx:278, cy:68 },
  "Normandie":                   { points:"88,58 218,42 218,95 182,128 148,138 88,118", cx:160, cy:90 },
  "Île-de-France":               { points:"248,112 295,112 304,153 272,162 244,152", cx:274, cy:135 },
  "Grand Est":                   { points:"335,22 425,32 445,52 448,162 402,180 362,168 342,130 335,105", cx:396, cy:108 },
  "Bretagne":                    { points:"42,152 142,132 182,128 172,172 140,198 95,208 42,192", cx:110, cy:172 },
  "Pays de la Loire":            { points:"88,118 182,128 198,172 188,228 148,248 92,232 72,192 88,158", cx:145, cy:192 },
  "Centre-Val de Loire":         { points:"182,128 248,112 244,152 278,162 294,212 268,248 212,258 188,228 198,172", cx:242, cy:198 },
  "Bourgogne-Franche-Comté":    { points:"294,152 362,168 402,180 418,288 392,318 348,328 302,298 280,262 280,228 294,212", cx:352, cy:252 },
  "Nouvelle-Aquitaine":          { points:"72,232 148,248 212,258 222,358 192,408 152,428 98,412 58,368 48,298", cx:142, cy:342 },
  "Auvergne-Rhône-Alpes":       { points:"268,248 294,212 280,262 302,298 348,328 392,318 422,398 382,428 312,428 262,388 252,318", cx:335, cy:352 },
  "Occitanie":                   { points:"98,412 152,428 192,408 222,358 262,388 312,428 332,478 278,508 198,508 138,488 92,458", cx:208, cy:458 },
  "Provence-Alpes-Côte d'Azur": { points:"382,428 422,398 452,378 472,418 462,478 422,498 362,488 332,478 312,428", cx:402, cy:453 },
  "Corse":                       { points:"445,478 460,458 475,488 480,528 464,542 448,532 438,508", cx:460, cy:503 },
};
    ...
    <div style={{minHeight:"100vh",background:"#04080f",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans','Segoe UI',sans-serif",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{textAlign:"center",maxWidth:720,padding:"0 32px",zIndex:10}}>
        ...
        <div className="land-sub" style={{opacity:visible?1:0,transform:visible?"none":"translateY(12px)",marginBottom:24}}>
          <span style={{background:"rgba(14,165,233,0.1)",border:"1px solid rgba(14,165,233,0.2)",borderRadius:20,padding:"5px 16px",color:"rgba(14,165,233,0.9)",fontSize:11,fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase"}}>SMO · OCP Group</span>
        </div>
...
function FranceMap({selectedRegion,onSelectRegion}){
  const [hovered,setHovered]=useState(null);
  const regions=Object.keys(FR_REGIONS_SVG);
  const hasData=r=>Object.keys(REGIONAL_DATA).includes(r);
  return(
    <svg viewBox="0 0 500 560" style={{width:"100%",maxWidth:400,height:"auto",display:"block",margin:"0 auto"}}>
      <defs>
        <filter id="glow"><feGaussianBlur stdDeviation="3" result="blur"/><feMerge><feMergeNode in="blur"/><feMergeNode in="SourceGraphic"/></feMerge></filter>
      </defs>
      {regions.map(name=>{
        const r=FR_REGIONS_SVG[name];
        const isSelected=selectedRegion===name;
        const isHovered=hovered===name;
        const hasD=hasData(name);
        const fill=isSelected?"#0ea5e9":isHovered?(hasD?"#1e3a5f":"#1a2030"):"#0f1e2e";
        const stroke=isSelected?"#0ea5e9":isHovered?"#334155":"#1e293b";
        const opacity=hasD?1:0.6;
        return(
          <g key={name} style={{cursor:hasD?"pointer":"default",opacity}}
            onClick={()=>hasD&&onSelectRegion(name)}
            onMouseEnter={()=>setHovered(name)}
            onMouseLeave={()=>setHovered(null)}>
            <polygon points={r.points} fill={fill} stroke={stroke} strokeWidth={isSelected?1.5:0.8} filter={isSelected?"url(#glow)":undefined} style={{transition:"fill 0.15s,stroke 0.15s"}}/>
            {/* Label only for hovered or selected — avoid clutter */}
            {(isHovered||isSelected)&&(
              <text x={r.cx} y={r.cy} textAnchor="middle" dominantBaseline="middle" fontSize={isSelected?8:7} fill="#f1f5f9" fontWeight={600} fontFamily="DM Sans,sans-serif" style={{pointerEvents:"none",userSelect:"none"}}>
                {name.length>18?name.split('-')[0]:name}
              </text>
            )}
            {/* Always show a small dot to mark each region */}
            {!isHovered&&!isSelected&&(
              <circle cx={r.cx} cy={r.cy} r={hasD?2.5:1.5} fill={hasD?"#0ea5e960":"#334155"} style={{pointerEvents:"none"}}/>
            )}
          </g>
        );
      })}
      ...
      <g transform="translate(10,530)">
        <circle cx={6} cy={6} r={4} fill="#0ea5e9"/>
        <text x={14} y={10} fontSize={8} fill="#64748b" fontFamily="DM Sans,sans-serif">Data available</text>
        <circle cx={80} cy={6} r={4} fill="#334155"/>
        <text x={88} y={10} fontSize={8} fill="#64748b" fontFamily="DM Sans,sans-serif">Coming soon</text>
      </g>
    </svg>
  );
}
...
function RegionalPage(){
  ...
  const [metric,setMetric]=useState("area"); // "area"|"production"|"yield"
  ...
  const chartData=regionData&&regionData[metric]?YEARS.map((y,i)=>({
    year:y,
    value:regionData[metric][activeCrop]?.[i]??0,
  })):[];

  const metricLabel={area:"Harvested Area (Ha)",production:"Production (tonnes)",yield:"Yield (t/Ha)"};
  const metricColor={area:"#0ea5e9",production:"#10b981",yield:"#f59e0b"};

  // Multi-crop comparison for current region
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div className="chart-grid-2">
        {/* Map */}
        <div className="card">
          <h3 className="card-title">Select French Region</h3>
          <FranceMap selectedRegion={selectedRegion} onSelectRegion={r=>{setSelectedRegion(r);setSelectedCrop("Wheat");}}/>
          <div style={{marginTop:12,display:"flex",flexWrap:"wrap",gap:6}}>
            {availableRegions.map(r=>(
              <button key={r} onClick={()=>{setSelectedRegion(r);setSelectedCrop("Wheat");}}
                style={{padding:"4px 10px",borderRadius:6,border:`1px solid ${selectedRegion===r?"#0ea5e9":"#1e293b"}`,background:selectedRegion===r?"#0ea5e920":"transparent",color:selectedRegion===r?"#0ea5e9":"#64748b",fontSize:10,cursor:"pointer"}}>
                {r.length>20?r.split('-')[0]:r}
              </button>
            ))}
          </div>
        </div>

        {/* Controls + single crop chart */}
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div className="card" style={{display:"flex",gap:12,flexWrap:"wrap",alignItems:"center"}}>
            <div>
              <p style={{color:"#64748b",fontSize:10,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:5}}>Crop</p>
              <select value={activeCrop} onChange={e=>setSelectedCrop(e.target.value)} style={{background:"#1e293b",border:"1px solid #334155",borderRadius:6,padding:"6px 10px",color:"#e2e8f0",fontSize:12,outline:"none"}}>
                {availableCrops.map(c=><option key={c}>{c}</option>)}
              </select>
            </div>
            <div>
              <p style={{color:"#64748b",fontSize:10,textTransform:"uppercase",letterSpacing:"0.07em",marginBottom:5}}>Metric</p>
              <div style={{display:"flex",gap:6}}>
                {Object.entries(metricLabel).map(([k,l])=>(
                  <button key={k} onClick={()=>setMetric(k)}
                    style={{padding:"5px 12px",borderRadius:6,border:`1px solid ${metric===k?metricColor[k]:"#1e293b"}`,background:metric===k?metricColor[k]+"20":"transparent",color:metric===k?metricColor[k]:"#64748b",fontSize:10,cursor:"pointer"}}>
          ...
          <div className="card">
            <h3 className="card-title">{activeCrop} — {metricLabel[metric]} · {selectedRegion}</h3>
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={chartData} margin={{left:0,right:10}}>
                <defs>
                  <linearGradient id="regionFill" x1="0" y1="0" x2="0" y2="1">
                ...
                <Area type="monotone" dataKey="value" name={metricLabel[metric]} stroke={metricColor[metric]} fill="url(#regionFill)" strokeWidth={2}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="card">
        <h3 className="card-title">Top 5 Crops — {metricLabel[metric]} · {selectedRegion}</h3>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={multiChart} margin={{left:0,right:20}}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e293b"/>
            <XAxis dataKey="year" tick={{fill:"#64748b",fontSize:9}}/>
            <YAxis tick={{fill:"#64748b",fontSize:9}} tickFormatter={v=>v>1000000?(v/1000000).toFixed(1)+"M":v>1000?(v/1000).toFixed(0)+"k":v}/>
            <Tooltip content={<CustomTooltip/>}/>
            <Legend wrapperStyle={{fontSize:10}}/>
            {availableCrops.slice(0,5).map((c,i)=><Line key={c} type="monotone" dataKey={c} stroke={cropColors[i]} strokeWidth={2} dot={false}/>)}
          </LineChart>
        </ResponsiveContainer>
      </div>
    </div>
...
function OverviewPage({data,region}){
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      ...
      {/* Product mix time series — from Excel */}
      <div className="card">
        <h3 className="card-title">Product Mix Evolution — France (kt P2O5)</h3>
        <p style={{color:"#475569",fontSize:11,marginBottom:12}}>Annual season data 2016–2023 · Source: Agreste / French Ministry of Agriculture</p>
        <ResponsiveContainer width="100%" height={220}>
          <AreaChart data={PRODUCT_MIX_DATA} margin={{left:0,right:10}}>
            <defs>
              {Object.entries(PM_COLORS).map(([k,c])=>(
                <linearGradient key={k} id={`pm_${k.replace("/","_").replace(" ","_")}`} x1="0" y1="0" x2="0" y2="1">
            ...
            <Legend wrapperStyle={{fontSize:10}}/>
            {Object.entries(PM_COLORS).map(([k,c])=>(
              <Area key={k} type="monotone" dataKey={k} stroke={c} fill={`url(#pm_${k.replace("/","_").replace(" ","_")})`} strokeWidth={2}/>
            ))}
          </AreaChart>
        </ResponsiveContainer>
      </div>
