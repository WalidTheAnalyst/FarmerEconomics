...
// ─── MARKET INTELLIGENCE DATA (keyed by region) ───────────────────────────
// To add a new country, add a matching key here with the same shape.
const MARKET_INTEL = {
  France: {
    kpis: [
    ...
    qualitative: {
      ...
      strategy: [
        ...
        { type: "risk",     label: "Regulatory headwinds",  text: "EU heavy metal and carbon regulations are tightening, which simultaneously creates risk for high-cadmium sources and opportunity for OCP's high-quality Moroccan P to gain regulatory tailwinds." },
      ],
    },
    cropAgronomy: [
...
// Shows full data when the selected region has an entry in MARKET_INTEL.
// Shows a clean placeholder for all other regions.
// To unlock a new country, add its key + data object to MARKET_INTEL above.
function MarketIntelligencePage({ region }) {
  const [qualTab, setQualTab] = useState("dynamics");
  ...
  if (!intel) {
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 340, gap: 14 }}>
        ...
        <p style={{ fontSize: 16, color: "#64748b" }}>No market intelligence data available for {region}.</p>
        <p style={{ fontSize: 13, color: "#334155" }}>Data is currently available for: {Object.keys(MARKET_INTEL).join(", ")}.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
        {intel.kpis.map((k, i) => (
          <KPICard key={i} label={k.label} value={k.value} sub={k.sub} accent={k.accent} />
        ))}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
          <h3 style={{ color: "#94a3b8", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>P2O5 consumption by product — kt (2022 → 2023)</h3>
        ...
        <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {intel.importOrigin.map((row, i) => {
              const maxVal = intel.importOrigin[0].value;
              return (
                <div key={i}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ color: "#94a3b8", fontSize: 12 }}>{row.name}</span>
                  ...
                  <div style={{ height: 8, background: "#1e293b", borderRadius: 4 }}>
                    <div style={{ height: "100%", width: `${(row.value / maxVal) * 100}%`, background: row.color, borderRadius: 4 }} />
                  </div>
                </div>
              );
            })}
      ...
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        <h3 style={{ color: "#94a3b8", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>Qualitative observations — {region}</h3>
        <div style={{ display: "flex", gap: 8, marginBottom: 18 }}>
          {[
            ...
            <button key={t.key} onClick={() => setQualTab(t.key)}
              style={{ padding: "5px 14px", borderRadius: 6, fontSize: 12, cursor: "pointer", border: `1px solid ${qualTab === t.key ? "#0ea5e9" : "#2e3244"}`, background: qualTab === t.key ? "#0ea5e915" : "transparent", color: qualTab === t.key ? "#0ea5e9" : "#64748b" }}>
              {t.label}
            </button>
          ))}
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          {intel.qualitative[qualTab].map((item, i) => (
            <div key={i} style={{ background: "#0a0f1a", borderRadius: 8, padding: "12px 14px", borderLeft: `3px solid ${qualBorder[item.type]}` }}>
              <p style={{ fontSize: 10, color: qualBorder[item.type], textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: 5 }}>{item.label}</p>
      ...
      {/* Crop agronomy table */}
      <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 20 }}>
        <h3 style={{ color: "#94a3b8", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 16 }}>Crop agronomy — current vs recommended P2O5 application (kg/ha)</h3>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #1e293b" }}>
              {["Crop", "Area (kha)", "Current P", "Rec. P", "Gap", "Priority product", "Fertilization logic"].map((h, i) => (
    ...
    <div style={{ minHeight: "100vh", background: "#060d1a", color: "#e2e8f0", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 113px)" }}>
        <div style={{ flex: 1, padding: 28, overflow: "auto" }}>
          ...
          {page === "intel"     && <MarketIntelligencePage region={region} />}
