...
function KPICard({ label, value, sub, accent }) {
  return (
    <div style={{ background: "#0f172a", border: `1px solid ${accent}30`, borderRadius: 12, padding: "16px 18px", flex: 1, minWidth: 130 }}>
      <p style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 6 }}>{label}</p>
      <p style={{ color: accent, fontSize: 22, fontWeight: 700, fontFamily: "'DM Mono', monospace", margin: 0 }}>{value}</p>
...
function OverviewPage({ data, region, crop }) {
  ...
  const regionAttractive = REGIONS.map(r => {
    ...
    return { region: r, avg: deltas.reduce((a, b) => a + b, 0) / deltas.length };
  });
  ...
  return (
    <div>
      ...
      <div className="chart-grid-2">
        ...
        <div className="card">
          <h3 className="card-title">Margin by Strategy — {crop} in {region}</h3>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={filtered} margin={{ left: 0, right: 10 }}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              <XAxis dataKey="strategy" tick={{ fill: "#64748b", fontSize: 9 }} />
              <YAxis tick={{ fill: "#64748b", fontSize: 10 }} />
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="margin" name="Gross Margin ($/ha)" radius={[4,4,0,0]}>
                {filtered.map((e, i) => <Cell key={i} fill={STRATEGY_COLORS[e.strategy] || "#64748b"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
  ...
  return (
    <div>
      <div className="simulator-grid">
        {/* Sliders */}
        <div className="card">
          <h3 className="card-title">Scenario Inputs — {crop} · {region}</h3>
          {realBase && <p style={{ color: "#10b981", fontSize: 11, marginBottom: 12, background: "#10b98110", borderRadius: 6, padding: "5px 10px", border: "1px solid #10b98120" }}>Base op. cost from real data: ${realBase.op_cost}/ha</p>}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <thead>
                <tr style={{ borderBottom: "1px solid #1e293b" }}>
                  {["Metric", ...scenarios.map(s => s.strategy)].map((h, i) => (
                    <th key={i} style={{ padding: "11px 14px", textAlign: i === 0 ? "left" : "right", color: i === 0 ? "#64748b" : Object.values(STRATEGY_COLORS)[i-1], fontSize: 10, textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 600, whiteSpace: "nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  <tr key={ri} style={{ borderBottom: "1px solid #0f1929", background: ri % 2 === 0 ? "#0a0f1a" : "#0f172a" }}>
                    <td style={{ padding: "9px 14px", color: "#94a3b8", fontSize: 11 }}>{label}</td>
                    {scenarios.map((s, si) => <td key={si} style={{ padding: "9px 14px", textAlign: "right", color: "#f1f5f9", fontFamily: "'DM Mono', monospace", fontSize: 12 }}>{fn(s)}</td>)}
                  </tr>
                ))}
                <tr style={{ background: "#0a1628", borderTop: "2px solid #1e293b" }}>
                  <td style={{ padding: "9px 14px", color: "#64748b", fontSize: 11 }}>Δ vs Baseline</td>
                  {scenarios.map((s, si) => {
                    const delta = s.margin - base.margin;
                    return <td key={si} style={{ padding: "9px 14px", textAlign: "right", fontFamily: "'DM Mono', monospace", fontSize: 12, fontWeight: 700, color: si === 0 ? "#475569" : delta >= 0 ? "#10b981" : "#f43f5e" }}>{si === 0 ? "—" : (delta >= 0 ? "+" : "") + fmt(delta)}</td>;
                  })}
                </tr>
              </tbody>
            </table>
          ...
          <div className="chart-grid-2">
            ...
            <div className="card" style={{ display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", gap: 10 }}>
              ...
              <p style={{ color: "#f59e0b", fontSize: 32, fontWeight: 700, fontFamily: "'DM Mono', monospace" }}>{breakEven} t/ha</p>
              <div style={{ width: "100%", background: "#1e293b", borderRadius: 8, padding: "10px", textAlign: "center" }}>
                <p style={{ color: yieldBoost / 100 * baseYield >= breakEven ? "#10b981" : "#f43f5e", fontWeight: 700, fontSize: 13 }}>
                  {yieldBoost / 100 * baseYield >= breakEven ? "✓ PAYS OFF" : "✗ NO BREAK-EVEN"}
                </p>
              </div>
            </div>
          </div>
...
function PLPage({ data, region, crop }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div className="card">
        <h3 className="card-title">Farm Cost Structure — {crop} in {region}</h3>
        {categories.map(cat => (
          <div key={cat.key} style={{ marginBottom: 8 }}>
            <div onClick={() => setExpanded(e => ({ ...e, [cat.key]: !e[cat.key] }))}
              style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer", padding: "7px 8px", borderRadius: 8, background: expanded[cat.key] ? "#1e293b" : "transparent" }}>
              <span style={{ color: cat.color, fontSize: 12 }}>{expanded[cat.key] ? "▼" : "▶"}</span>
              <span style={{ color: "#e2e8f0", fontSize: 13, flex: 1 }}>{cat.label}</span>
              <div style={{ width: 100, background: "#1e293b", borderRadius: 4, height: 5 }}>
                <div style={{ width: `${cat.share}%`, background: cat.color, borderRadius: 4, height: "100%" }} />
              </div>
              <span style={{ color: "#94a3b8", fontSize: 11, width: 34, textAlign: "right" }}>{cat.share}%</span>
              <span style={{ color: "#f1f5f9", fontFamily: "'DM Mono', monospace", fontSize: 12, width: 60, textAlign: "right" }}>${cat.value}</span>
            </div>
            {expanded[cat.key] && (
              <div style={{ marginLeft: 28, padding: "7px 10px", background: "#0a0f1a", borderRadius: 8, fontSize: 11, color: "#64748b" }}>
                {cat.key === "fert" ? `Blended: $${cat.value}/ha → Separated: $${Math.round(cat.value * 0.95)}/ha (≈5% reduction)` : `Standard allocation for ${crop.toLowerCase()} in ${region}.`}
              </div>
            )}
          </div>
        ))}
      ...
      <div className="card">
        <h3 className="card-title">Margin Sensitivity to Fertilizer Price — {crop} · {region}</h3>
        <ResponsiveContainer width="100%" height={200}>
...
function AgronomyPage({ data, region, crop }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      {base && (
        <div className="card" style={{ padding: "12px 16px", flexDirection: "row", flexWrap: "wrap", gap: 16, display: "flex", alignItems: "center" }}>
          <span style={{ color: "#64748b", fontSize: 12 }}>Context:</span>
          <span style={{ color: "#0ea5e9", fontSize: 13, fontWeight: 600 }}>{crop} in {region}</span>
          <span style={{ color: "#64748b", fontSize: 12 }}>pH: <span style={{ color: "#f59e0b", fontFamily: "'DM Mono', monospace" }}>{ph}</span></span>
      ...
      <div className="chart-grid-2">
        <div className="card">
          ...
          <p style={{ color: "#475569", fontSize: 11, marginBottom: 12 }}>Separation advantage highest at pH &gt; 7.5</p>
          <ResponsiveContainer width="100%" height={180}>
            <LineChart data={phData}>
              ...
              <ReferenceLine x={String(ph.toFixed(1))} stroke="#f59e0b60" strokeDasharray="4 4" />
        ...
        <div className="card">
          ...
          <p style={{ color: "#475569", fontSize: 11, marginBottom: 12 }}>Separation shifts the yield plateau upward</p>
          <ResponsiveContainer width="100%" height={180}>
        ...
        <div className="card">
          ...
          <p style={{ color: "#475569", fontSize: 11, marginBottom: 12 }}>Strategy gap narrows above 2.5% OM</p>
          <ResponsiveContainer width="100%" height={180}>
        ...
        <div className="card">
          <h3 className="card-title">Key Biochemical Mechanisms</h3>
          {[
            { title: "P Fixation in Calcareous Soils", body: "At pH > 7.5, Ca²⁺ precipitates MAP-released phosphate within hours. TSP maintains a lower pH microzone around the granule, delaying fixation.", accent: "#f59e0b" },
            { title: "NH₄⁺ Interference in Blends",   body: "MAP/DAP release N and P simultaneously. High NH₄⁺ suppresses root P uptake early season. Separation removes this competition.", accent: "#a78bfa" },
            { title: "Placement & Timing",             body: "Banded TSP subsurface puts P directly in the root zone. Broadcast MAP loses 15–35% to surface fixation.", accent: "#0ea5e9" },
          ].map((item, i) => (
            <div key={i} style={{ borderLeft: `2px solid ${item.accent}`, paddingLeft: 10, marginBottom: 12 }}>
              <p style={{ color: item.accent, fontSize: 12, fontWeight: 600, marginBottom: 2 }}>{item.title}</p>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      <div className="card" style={{ border: "1px solid #0ea5e930" }}>
        <h3 style={{ color: "#0ea5e9", fontSize: 12, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 16 }}>◈ Model Insights — {crop} · {region}</h3>
        {insights.length ? insights.map((ins, i) => (
          <div key={i} style={{ display: "flex", gap: 12, marginBottom: 12, padding: "11px 13px", background: "#0a0f1a", borderRadius: 8 }}>
            <span style={{ color: "#0ea5e9", fontSize: 14, marginTop: 2, flexShrink: 0 }}>→</span>
            ...
            {allInsights.sort((a, b) => b.delta - a.delta).map((row, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #0f1929", background: i % 2 === 0 ? "#0a0f1a" : "#0f172a" }}>
                ...
                <td style={{ padding: "8px 12px", textAlign: "right" }}>
                  <span style={{ padding: "2px 8px", borderRadius: 20, fontSize: 10, background: row.delta > 30 ? "#10b98120" : row.delta > 0 ? "#0ea5e920" : "#f43f5e20", color: row.delta > 30 ? "#10b981" : row.delta > 0 ? "#0ea5e9" : "#f43f5e", whiteSpace: "nowrap" }}>
    ...
    <div style={{ minHeight: "100vh", background: "#060d1a", color: "#e2e8f0", fontFamily: "'DM Sans', 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* ── Desktop sidebar ── */
        .app-sidebar { width: 200px; flex-shrink: 0; border-left: 1px solid #1e293b; padding: 16px; background: #0a0f1a; }
        .sidebar-overlay { display: none; }

        /* ── Desktop sub-nav ── */
        .subnav { background: #0a0f1a; border-bottom: 1px solid #0f1929; padding: 0 20px; display: flex; overflow-x: auto; }
      ...
      <div style={{ borderBottom: "1px solid #1e293b", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 50, background: "#0a0f1a", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 7, height: 7, borderRadius: "50%", background: "#0ea5e9", boxShadow: "0 0 8px #0ea5e9", flexShrink: 0 }} />
          <span style={{ fontWeight: 700, fontSize: 14, letterSpacing: "-0.02em", color: "#f1f5f9" }}>PhosStratOS</span>
          <span style={{ color: "#334155", fontSize: 11, display: "none" }} className="subtitle-pipe">|</span>
        </div>
      ...
      <div style={{ background: "#080e1a", borderBottom: "1px solid #1e293b", padding: "0 16px", display: "flex", alignItems: "center", overflowX: "auto" }}>
        {[
          ...
          <button key={s.key} onClick={() => setSection(s.key)}
            style={{ padding: "10px 16px", background: "transparent", border: "none", borderBottom: section === s.key ? `2px solid ${s.color}` : "2px solid transparent", color: section === s.key ? "#f1f5f9" : "#475569", fontSize: 12, fontWeight: section === s.key ? 700 : 400, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
        ...
        <button onClick={() => setSection("atlas")}
          style={{ marginLeft: "auto", padding: "10px 16px", background: "transparent", border: "none", borderBottom: section === "atlas" ? "2px solid #a78bfa" : "2px solid transparent", color: section === "atlas" ? "#f1f5f9" : "#475569", fontSize: 12, fontWeight: section === "atlas" ? 700 : 400, cursor: "pointer", display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
      ...
      {section !== "atlas" && (
        <>
          ...
          <div className="subnav">
            {quantPages.map(p => {
              const active = quantPage === p.key;
              return (
                <button key={p.key} onClick={() => setQuantPage(p.key)}
                  style={{ padding: "8px 14px", background: "transparent", border: "none", borderBottom: active ? "2px solid #0ea5e9" : "2px solid transparent", color: active ? "#0ea5e9" : "#64748b", fontSize: 12, fontWeight: active ? 600 : 400, cursor: "pointer", whiteSpace: "nowrap" }}>
                  {p.short}
                </button>
              );
            })}
          ...
          <div style={{ display: "flex", minHeight: "calc(100dvh - 130px)" }}>
            <div className="page-content" style={{ flex: 1, padding: 20, overflow: "auto", minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18, flexWrap: "wrap" }}>
                ...
                <h1 style={{ fontSize: 16, fontWeight: 700, color: "#f1f5f9", letterSpacing: "-0.02em" }}>
                  {quantPages.find(p => p.key === quantPage)?.label}
                </h1>
                <span style={{ color: "#334155", fontSize: 12 }}>
                  {isOverview ? `all crops · ${region}` : `${crop} · ${region}`}
                </span>
              </div>

              {quantPage === "overview"  && <OverviewPage  data={SAMPLE_DATA} region={region} crop={crop} />}
              {quantPage === "simulator" && <SimulatorPage data={SAMPLE_DATA} region={region} crop={crop} />}
              {quantPage === "pl"        && <PLPage        data={SAMPLE_DATA} region={region} crop={crop} />}
              {quantPage === "agronomy"  && <AgronomyPage  data={SAMPLE_DATA} region={region} crop={crop} />}
              {quantPage === "insights"  && <InsightsPage  data={SAMPLE_DATA} region={region} crop={crop} />}
            </div>
            ...
            <div className={`app-sidebar ${sidebarOpen ? "open" : ""}`}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                ...
                <button onClick={() => setSidebarOpen(false)} style={{ background: "transparent", border: "none", color: "#475569", fontSize: 16, cursor: "pointer", padding: "0 2px" }}>✕</button>
              </div>
